				...
				<div class="div2_1">
					...
					if(list.size()==0){%>
						...
						for(int i = 0 ; i < list.size(); i++){%>
			                ...
			                %>
								<td style="color: red;"
									onclick="newData('<%=list.get(i).getWorkCode()%>');"><b>미등록</b></td>
								<%
			                		break;
			                	case 1:
							%>
								<td onclick="showData('<%=list.get(i).getWorkCode()%>');"
									style="color: blue;"><b>확인/다운로드</b></td>
								<%
									break;
			                	default:
			                		break;
							...
							<tr id="btm_tr">
								<th>
									<%if(list.size()==0){ %> 작업 없음 <%}else { %> 총 작업 : <%=list.size() %>
									<%} %>
								</th>
								<th>-</th>
								<th>-</th>
					...
					<div class="div2_1_3">
						<p>※ 첨부파일을 등록하지 않으면, [미등록] 으로 표시됩니다. 완료 작업에 대한 증빙 자료는 [자료 요청]을
							통해 요청 할 수 있습니다.</p>
						<br />
