...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%List<ClientInfoVO> list = (List)request.getAttribute("cInfo"); 
      ...
      phone = vo.getFieldManagerPhone().split("-");
      cell = vo.getFieldManagerCell().split("-");
      workTime = vo.getWorkTime().split(":");
      if(vo.getFieldManagerMail().equals("없음")){
    	  vo.setFieldManagerMail("");
      }
    %>
...
<script type="text/javascript">
	...
	function sltSet() {
		var sltData = '<%=vo.getRv()%>';
		var indexNum = 0;
		for(var i = 0 ; i < document.getElementById("rv").options.length; i++){
			if(sltData==document.getElementById("rv").options[i].value){
				sltData==document.getElementById("rv").options[i].setAttribute('selected','selected');
				indexNum++;
			}
		}
		if(indexNum==0){
			document.getElementById("rv").options[8].setAttribute('selected','selected');
			document.getElementById("rvDirect").style.display = "inline";
			document.getElementById("rvDirect").value=sltData;
		}
	}
</script>
<style>
...
<body onload="sltSet();">
	...
	<div class="clientInput">
		<form action="modifyWorkAct.do" id="form" name="form">
			<input type="hidden" name="clientCode" id="clientCode" value="<%=vo.getClientCode()%>"/>
			<label for="clientCpName">거래처&nbsp;명</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" readonly="readonly" name="clientCpName" value="<%=vo.getClientCPName() %>" id="clientCpName" onclick="altClick();"/><br />
			<label for="clientManager">거래처&nbsp;담당자</label>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" readonly="readonly" name="clientManager" value="<%=vo.getClientManager() %>" id="clientManager" onclick="altClick();"/><br />
			<label for="clientPhone">거래처&nbsp;전화번호</label>&nbsp;<input type="text" readonly="readonly" name="clientPhone" id="clientPhone" value="<%=vo.getClientPhone() %>" onclick="altClick();"/><br />
			<label for="relative">&nbsp;&nbsp;&nbsp;비&nbsp;&nbsp;&nbsp;&nbsp;고&nbsp;</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" readonly="readonly" name="relative" id="relative" value="(개발자 메세지)테이블 수정 요망" onclick="altClick();"/><br />
			<hr />
			<input type="hidden" id="workCode" name="workCode" value="<%=vo.getWorkCode()%>"/>
			<table>
				<tr>
					<td>현장명</td>
					<td><input type="text" name="workField" id="workField" value="<%=vo.getWorkField() %>" maxlength="20"/></td>
				</tr>
				<tr>
					<td>필요 차량</td>
					<td>
				...
				<tr>
					<td>현장 책임자</td>
					<td><input type="text" name="fieldManager" id="fieldManager" value="<%=vo.getFieldManager() %>" maxlength="7"/></td>
				</tr>
				<tr>
					<td>현장 책임자 전화번호</td>
					<td>
						<input type="hidden" id="fieldManagerPhone" name="fieldManagerPhone" value="<%=vo.getFieldManagerPhone()%>"/><input type="text" name="fieldManagerPhone01" id="fieldManagerPhone01" value="<%=phone[0] %>" maxlength="4"/>-<input type="text" name="fieldManagerPhone02" id="fieldManagerPhone02" value="<%=phone[1] %>" maxlength="4"/>-<input type="text" name="fieldManagerPhone03" value="<%=phone[2] %>" id="fieldManagerPhone03" maxlength="4"/><br />
					</td>
				</tr>
				<tr>
					<td>현장 책임자 핸드폰 번호</td>
					<td>
						<input type="hidden" id="fieldManagerCell" name="fieldManagerCell" value="<%=vo.getFieldManagerCell() %>"/><input type="text" name="fieldManagerCell01" id="fieldManagerCell01" value="<%=cell[0] %>" maxlength="3"/>-<input type="text" name="fieldManagerCell02" value="<%=cell[1] %>" id="fieldManagerCell02" maxlength="4"/>-<input type="text" name="fieldManagerCell03" value="<%=cell[2] %>" id="fieldManagerCell03" maxlength="4"/><br />		
					</td>
				</tr>
				<tr>
					<td>현장 책임자 이메일[선택]</td>
					<td><input type="text" name="fieldManagerMail" id="fieldManagerMail" value="<%=vo.getFieldManagerMail() %>"/><br /></td>
				</tr>
				<tr>
					<td>현장 주소</td>
					<td><input type="text" name="fieldAdd01" id="fieldAdd01" maxlength="33" value="<%=vo.getFieldAdd01() %>"/><br /></td>
				</tr>
				<tr>
					<td>현장 상세 주소</td>
					<td><input type="text" name="fieldAdd02" id="fieldAdd02" maxlength="19" value="<%=vo.getFieldAdd02() %>"/><br /></td>
				</tr>
				<tr>
					<td>작업 금액</td>
					<td><input type="text" name="workAmount" id="workAmount" maxlength="10" value="<%=vo.getWorkAmount() %>"/><br /></td>
				</tr>
				<tr>
					<td>작업 날짜</td>
					<td><input type="date" name="dateWork" id="dateWork" placeholder="클릭하면 달력이 생성됩니다." value="<%=vo.getWorkDate() %>"/><br /></td>
				</tr>
				<tr>
					<td>작업 시간</td>
					<td><input type="hidden" id="workTime" name="workTime" value="<%=vo.getWorkTime()%>"/><input type="text" name="workTime01" id="workTime01" value="<%=workTime[0] %>" maxlength="2"/>:<input type="text" name="workTime02" id="workTime02" value="<%=workTime[1] %>" maxlength="2"/><br /></td>
				</tr>
				<tr>
					<td>작업 요청 사항</td>
					<td><input type="text" name="workRequests" id="workRequests" maxlength="40" value="<%=vo.getWorkRequests() %>"/><br /></td>
				</tr>
				<tr>
					<td>비고</td>
					<td><input type="text" name="relative" id="relative" maxlength="40" value="<%=vo.getRelative() %>"/><br /></td>
				</tr>
