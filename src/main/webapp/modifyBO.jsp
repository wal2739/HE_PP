...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%
		...
		try{
			...
			boMail = boInfo02.getBoMail().split("@");
			boAdd01 = boInfo02.getBoAdd01().split("/");
...
<script type="text/javascript">
	...
	$('document').ready(function() {
      ...
      $('document').ready(function () {
  		...
  		var IndexNum = 0;
  		for(var i = 0 ; i < sido1.options.length; i++){
  			if(boAdd01Data01==sido1.options[i].value){
  				IndexNum = i ;
  				break;
  			}
  		}
  		sido1.value = sido1.options[IndexNum].value;

  		
  		////////////ㅁ 초기 설정 jQuery ㅁ/////////////
  		var area = "area"+IndexNum;
        var $gugun = $(this).next();
        $("option",$gugun).remove(); 
        if(area == "area0")
        $gugun.append("<option value=''>구/군 선택</option>");
        ...
        $.each(eval(area), function() {
  		...
  		for(var i = 0 ; i < gugun1.options.length; i++){
  			if(boAdd01Data02==gugun1.options[i].value){
  				IndexNum = i ;
  				break;
  			}
  		}
  		...
  		gugun1.value = gugun1.options[IndexNum].value; 
  		
  		IndexNum=0;
      	var boAccBackData = '<%=boInfo02.getBoAccBack()%>';
		var boAccBack = document.getElementById('boAccBack');
		for(var i = 0 ; i < boAccBack.options.length; i++){
  			if(boAccBackData==boAccBack.options[i].value){
  				IndexNum = i ;
  				break;
  			}
  		}
		boAccBack.value = boAccBack.options[IndexNum].value; 
		
		IndexNum=0;
      	var mailAddData = '<%=boMail[1]%>';
		var mailAdd = document.getElementById('mailAdd');
		for(var i = 0 ; i < mailAdd.options.length; i++){
  			if(mailAddData==mailAdd.options[i].value){
  				IndexNum = i ;
  				break;
  			}
  		}
		mailAdd.value = mailAdd.options[IndexNum].value; 
      });
...
<body onload="boCheck();">
  ...
  <main>
	<form action="modifyBOAct.do" name="forms" id="forms" onsubmit="return validate();">    	
    	<div class="mainDiv">
      <div class="div1">
          ...
          <div class="div2_1_1">
            <table id="table01" name="table01" class="table01">
              ...
              <tr>
                <th>사업자 번호*</th>
                <td colspan="3">
                  <input type="text" name="boNumber01" value="<%=boNumber[0] %>" id="boNumber01" maxlength="3"/><b>&nbsp;&nbsp;-&nbsp;&nbsp;</b>
                  <input type="text" name="boNumber02" value="<%=boNumber[1] %>" id="boNumber02" maxlength="2"/><b>&nbsp;&nbsp;-&nbsp;&nbsp;</b>
              ...
              <tr>
                <th>업체명</th>
                <td colspan="1"><input type="text" value="<%=boInfo02.getCpName()%>" name="cpName" id="cpName" maxlength="15"/></td>
                <th>사업자명*</th>
                <td><input type="text" value="<%=boInfo02.getBoName() %>" name="boName" id="boName" onkeyup="boName_BoAccName();"/></td>
              </tr>
              <tr>
                <th>업종*</th>
              ...
              <tr>
                <th>주민등록번호*</th>
                <td colspan="3">
                  <input type="text" value="<%=boRRN[0] %>" name="boRrn01" id="boRrn01" maxlength="6"/><b>&nbsp;&nbsp;-&nbsp;&nbsp;</b>
                  <input type="password" value="<%=boRRN[1] %>" name="boRrn02" id="boRrn02" maxlength="7"/>
              ...
              <tr>
                <th>전화번호*</th>
                <td>
			    	<input type="text" value="<%=boPhone[0]%>" maxlength="3" name="boPhone01" id="boPhone01"/>-
	                <input type="text" value="<%=boPhone[1]%>" maxlength="4" name="boPhone02" id="boPhone02"/>-
	                <input type="text" value="<%=boPhone[2]%>" maxlength="4" name="boPhvone03" id="boPhone03"/>
	                <input type="hidden" name="boPhone" id="boPhone"/>
			    </td>
			    
                <th>핸드폰번호*</th>
                <td colspan="2">
	                <input type="text" value="<%=boCell[0]%>" maxlength="3" name="boCell01"id="boCell01"/>-
	                <input type="text" value="<%=boCell[1]%>" maxlength="4" name="boCell02"id="boCell02"/>-
	                <input type="text" value="<%=boCell[2]%>" maxlength="4" name="boCell03"id="boCell03"/>
              ...
              <tr class="mail_tr">
                ...
                <td colspan="3">
                  ...
                  <input type="text" value="<%=boMail[0]%>" name="boMail01" id="boMail01"><b>&nbsp;@&nbsp;</b>
                  <input type="text" value="<%=boMail[1]%>" name="boMail02" id="boMail02" readonly="readonly">
              ...
              <tr class="address02_text">
                <th>상세주소</th>
                <td colspan="3">
                	<input type="text" value="<%=boInfo02.getBoAdd02() %>" name="boAdd02" id="boAdd02"/>
                </td>
              </tr>
              <tr>
                <th>계좌은행*</th>
              ...
              <tr>
                <th>계좌번호*</th>
                <td colspan="3">
                	<input type="text"value="<%=boInfo02.getBoAcc() %>"  name="boAcc" id="boAcc" placeholder="- 빼고 작성" width="200px"/>&nbsp;&nbsp;예금주 : &nbsp;
                	<input type="text" readonly="readonly" id="boAccName" value="<%=boInfo02.getBoName() %>">
